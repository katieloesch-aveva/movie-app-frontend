...
export class LandingPageComponent implements OnInit {
  ...
  private getPosterUrl(item: any): string {
    if (item?.poster_path) {
      // console.log(
      //   `https://image.tmdb.org/t/p/w${this.imgWidth}${item.poster_path}`
      // );
      return `https://image.tmdb.org/t/p/w${this.imgWidth}${item.poster_path}`;
    } else if (item?.poster_link) {
      return item.poster_link;
    } else {
      return 'assets/placeholder-img.png';
    }
  }
  ...
  fetchTrendingFilms(): void {
    ...
    this.http.get<any>(this.tmdbUrl).subscribe({
      next: (res) => {
        ...
        this.trendingFilms = results.map((film: any) => ({
          ...film,
          poster_url: this.getPosterUrl(film),
          selected: false,
        }));
      },
      error: (err) => {
